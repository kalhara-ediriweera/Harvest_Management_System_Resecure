...
const SambaStockDetail = () => {
  ...
  const getProductImage = (variety) => {
    ...
    return varietyMap[variety] || '/shopImg/samba.jpg';
  };
  ...
  const handleAddToCart = (stock) => {
    ...
    if (stock.quantity < quantity) {
      setNotification({
        message: `Sorry, only ${stock.quantity} ${stock.quantityUnit} available!`,
        type: 'error'
      });
  ...
  // Navigate to variety page
  const navigateToVariety = (variety) => {
    const formattedVariety = variety.toLowerCase().replace(' ', '-');
    navigate(`/${formattedVariety}-stock-detail`);
    setShowSearchResults(false);
  };

  // Get variety description based on name
  const getVarietyDescription = () => {
    return 'Fragrant, fine grain, needs longer growth period. A premium variety with exceptional taste and aroma, perfect for special occasions.';
  };
  ...
  const getCropTypeFeatures = (type) => {
    if (type.toLowerCase() === 'rice') {
      return [
        ...
        { icon: <FaLeaf className="text-green-500" />, text: 'Premium Grade' },
        { icon: <FaStar className="text-yellow-500" />, text: 'Aromatic' }
      ];
    } else {
      return [
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <section className="mb-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {['Nadu', 'BG352', 'Red Rice', 'Pachcha'].filter(v => v !== currentVariety).slice(0, 4).map((relatedVariety, idx) => (
              <Link 
...
export default SambaStockDetail;
